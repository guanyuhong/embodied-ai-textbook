...
传统人工智能课程常以“数据—模型—算法”为主线展开，例如监督学习与深度学习的基本范式、计算机视觉与自然语言处理的典型任务、以及面向推理与规划的问题建模。这些方法擅长在给定数据上完成识别、预测或生成，但它们通常不直接改变外部环境，也不需要持续承担行动后果。具身智能关注的问题有所不同：智能体不仅要理解环境，还要在环境中行动，并根据行动后的反馈修正自身行为。
...
从发展脉络看，具身智能并非凭空出现：早期研究更多强调符号推理与规则系统，随后行为主义与机器人研究推动了“在环境中行动”的观点；进入深度学习与强化学习阶段，学习方法逐步具备在感知与控制中形成策略的能力；近年大语言模型与多模态模型带来更强的任务理解与规划能力，使“具身智能体”可以在更复杂任务中组织多步行动。对于高校导论课程而言，重要的不是逐一罗列最新系统或榜单，而是把握可复用的通用框架与边界条件。

本章作为全书导论，重点完成三件事：给出具身智能的基本定义，建立感知-规划-行动-反馈闭环，说明本课程从概念、系统、算法、仿真到项目实践的学习路径。RaysTwins 等仿真平台在本书中仅作为教学辅助工具，用于帮助读者观察任务闭环与实验过程；涉及具体平台接口、任务包格式和部署细节的内容将在后续章节或实验手册中展开，资料缺失处统一标注“需平台方补充”。

## 2. 学习目标

学完本章后，读者应能：

1. 说明具身智能的基本含义，并辨析具身智能、普通人工智能、机器人系统与 AI Agent（人工智能智能体）的差异。
2. 理解身体、环境、任务、观测、状态、动作、目标与反馈在具身智能系统中的作用。
3. 使用“感知-规划-行动-反馈闭环”描述一个简单的具身智能任务。
4. 识别具身智能问题域中的正例、反例与边界例。
5. 初步说明仿真平台在具身智能教学中的作用与适用边界。
...
许多人工智能系统主要处理静态数据或离线数据。这类系统的重要特点是：输入与输出之间存在相对明确的数据接口，模型通常不直接改变外部环境。
...
![图 1-1 从普通 AI 到具身智能的能力递进](../../figures/ch01/fig1-1_ai_to_embodied_ai.svg)
...
“具身”强调智能不是孤立发生在模型内部，而是发生在智能体与环境交互的过程中。一个具身智能任务至少包含身体、环境、任务与反馈四类要素。
...
![图 1-3 具身智能、机器人与 AI Agent 的关系](../../figures/ch01/fig1-3_robot_agent_embodied_relation.svg)

图 1-3 展示了具身智能、机器人系统与 AI Agent 的关系。机器人系统提供身体和执行载体，AI Agent 提供目标驱动的任务组织能力，具身智能则强调身体与环境约束中的连续行动反馈闭环。

### 5.5 关键术语分层（导览）

为避免将“模型名词”与“系统能力”混为一谈，本书对相关术语做如下分层理解：

1. 大语言模型（LLM）主要面向语言理解与生成，可作为任务理解、计划生成与工具调用的上层模块。
2. 视觉语言模型（VLM）将视觉观测与语言指令联系起来，常用于场景理解、目标识别与语义描述。
3. 视觉—语言—动作模型（VLA）尝试将观测、指令与动作输出统一到同一范式，适合用来理解“从指令到行动”的研究方向，但其工程成熟度与适用边界需要谨慎评估。

本章仅用这些术语说明系统构成的层次关系；与 VLA、世界模型等更前沿的讨论将在第 11 章展开。
...
![图 1-4 校园导览机器人任务示意图](../../figures/ch01/fig1-4_campus_navigation_task.svg)
...
![图 1-5 本课程实验平台层级](../../figures/ch01/fig1-5_teaching_platform_layers.svg)
...
figures_planned:
  - 图 1-1 从普通 AI 到具身智能的能力递进 -> figures/ch01/fig1-1_ai_to_embodied_ai.mmd / figures/ch01/fig1-1_ai_to_embodied_ai.svg
  - 图 1-2 具身智能的感知-规划-行动-反馈闭环 -> figures/ch01/fig1-2_embodied_loop.mmd / figures/ch01/fig1-2_embodied_loop.svg
  - 图 1-3 具身智能、机器人与 AI Agent 的关系 -> figures/ch01/fig1-3_robot_agent_embodied_relation.mmd / figures/ch01/fig1-3_robot_agent_embodied_relation.svg
  - 图 1-4 校园导览机器人任务示意图 -> figures/ch01/fig1-4_campus_navigation_task.mmd / figures/ch01/fig1-4_campus_navigation_task.svg
  - 图 1-5 本课程实验平台层级 -> figures/ch01/fig1-5_teaching_platform_layers.mmd / figures/ch01/fig1-5_teaching_platform_layers.svg
style_boxes_used:
  - 定义 1-1 具身智能
  - 定义 1-2 具身智能体
  - 例 1-1 校园导览机器人
...
human_confirm:
  - 拓展阅读条目与链接需由 Human Editor 逐条核验（references/sources.yaml 中均为 verified: false）。
sources_cited:
  - sutton_barto_rl
  - lynch_park_modern_robotics
  - thrun_probabilistic_robotics
  - ros2_docs
-->
